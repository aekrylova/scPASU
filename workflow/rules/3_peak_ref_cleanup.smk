...
rule cellranger_make_scPASU_ref:
    ...
    shell:  
        ...
        if [ {params.PAS_filtering} == 'true' ]
            then file_prefix={params.compartment}
        else
            file_prefix={params.compartment}_noPASfiltering
        fi

        ref={input.work_dir}/{input.indir}/${{file_prefix}}_final_peak_universe_updated.txt
        gtf={input.work_dir}/{input.indir}/${{file_prefix}}_final_peak_universe_updated.gtf

        echo Convert final polyA site reference to gtf file format... &> {log}
        Rscript {input.work_dir}/scripts/ref2gtf.R -b $ref -f ${{file_prefix}} -o {input.indir}/ &>> {log}
